---
title: "Understanding Multiple Regression Results"
date: "`r Sys.Date()`"
header-includes:
   - \usepackage{xcolor}
...
In this set of notes, we will again examine the question of whether education level is related to income using the *riverside.csv* data from @Lewis-Beck:2016. In particular, we will re-examine the multiple regression model and try to gain a better understanding of *statistical control*.
...
ggplot(data = city, aes(x = education, y = income)) +
  ...
  scale_y_continuous(name = "Predicted income", breaks = seq(from = 37000, to = 45000, by = 1000), limits = c(37000, 45000)) +
  xlim(9, 13)
```
...
ggplot(data = city, aes(x = seniority, y = income)) +
  ...
  scale_y_continuous(name = "Predicted income", breaks = seq(from = 40000, to = 50000, by = 1000), limits = c(40000, 50000)) +
  xlim(5, 9)
```
...
```{r echo=FALSE, fig.height=6, fig.width=8, out.height='4in'}
...
plotData$seniority =  factor(plotData$seniority,
    levels = c(10, 15, 20),
    labels = c("Low seniority", "Moderate seniority", "High Seniority")
    )

ggplot(data = plotData, aes(x = education, y = yhat, group = seniority)) +
...
To start this process, we need to basically set up data that includes the range of values (8--24) for education for ALL three values of seniority. To do this we will use the `expand.grid()` function rather than the `data.frame()` function. 
...
```{r fig.width=8, fig.height=6, out.width='5in'}
ggplot(data = plotData, aes(x = education, y = yhat, group = seniority2, color = seniority2)) +
  geom_line() +
  theme_bw() +
  xlab("Education level") +
  ylab("Predicted income") +
...
Rather than create different objects, this time I am going to use dplyr's piping syntax throughout the whole process.
...
expand.grid(
  ...
  mutate( 
    education2 = factor(education, levels = c(12, 16), labels = c("High school", "College") ) 
    ) %>%
...
Figure 2. *Predicted income, controlling for education level, as a function of seniority level. Education was set to its mean value of 16 yrs.*
