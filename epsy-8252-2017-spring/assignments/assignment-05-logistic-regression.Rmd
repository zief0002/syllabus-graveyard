...
2. Fit two logistic models to the data using religious service attendance. In the first model, only include the linear effect of attendance. In the second model, include both a linear and quadratic effect to predict variation in support for same-sex marriage. Which model should bebe adopted (linear or quadratic? Justify your response by providing any statistical evidence you used in reaching your decision.

3. Create a table of model results. Add the results from the fitted model you adopted to this table. (You will add other models to this table as you progress through the assignment.) For model-level summaries, include the AICc value, and at least one pseudo-$R^2$ value.
...
4. Write the fitted equation for the adopted model.
...
6. Use the `CrossTables()` function in the **gmodels** package (or some other equivalent function) to summarize the cross-tabulations between support of same-sex marriage and religous denominations. Are there sample differences in the proportion of people who support same-sex marriage between the denominations? Explain.
...
8. Interpret the effect associated with the `Jewish` coefficient in terms of (a) log-odds, and (b) odds.
...
10. To help think about which covariates should be included in your model, create a correlation matrix of the outcome, and all of three covariates. By referring to this matrix, clearly indicate which covariates should be included, and which will not. Be sure to acknoledge  both correlations with the outcome AND with the other covariates in your justification.
...
12. Create a plot that visually displays the results of your final fitted model. Be sure to visually show the effects the focal predictors (religious service attendance and denomination). Also show any pertinent covariates you think are necessary to include. (Think about how the inclusion of the covariates help readers better understand the effects ofthe focal predictors.)
