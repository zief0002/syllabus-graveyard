...
To compute the residuals, we will use the `fortify()` function from the **ggplot2** package. We will also write those results into an object so we can compute on it later.
...
Since the $t$-distribution is also referred to as *Student's distribution*, this transformation of the residuals is called *studentizing*. What studentizing does for us is to put the residuals on a scale that uses the standard error. This allows us to judge whether particular residuals that look extreme (either highly positive or negative) are actually extreme or not. The studentized residuals are given in the fotified output as `.stdresid`.

```{r eval=FALSE}
# Plot the studentized residuals versus education level
ggplot(data = out_1, aes(x = education, y = .stdresid)) +
  geom_point() +
  theme_bw() +
  geom_hline(yintercept = 0) +
...
sm.density(out_1$.stdresid, xlab = "Studentized residuals", model = "normal")
```
...
ggplotted = ggplot(data = out_1, aes(x = education, y = .stdresid)) +
  geom_point() +
  theme_bw() +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = -2, linetype = "dotted") +
...
```{r}
out_1 %>% filter(.stdresid <= -2 | .stdresid >= 2)
```
...
pushViewport(vp.1) # access the first position
ggplotted = ggplot(data = out_2, aes(x = .fitted, y = .stdresid)) +
  geom_point() +
  theme_bw() +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = -2, linetype = "dotted") +
...
extreme = out_1 %>% filter(.stdresid <= -2 | .stdresid >= 2)
nonextreme = out_1 %>% filter(.stdresid > -2 & .stdresid < 2)

# Plot using text for the extreme observations and points for the non-extreme
ggplot(data = extreme, aes(x = .fitted, y = .stdresid)) +
